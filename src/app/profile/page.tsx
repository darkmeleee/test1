...
export default function ProfilePage() {
  ...
  const ordersQuery = api.orders.getUserOrders.useQuery(undefined, {
    enabled: hasDbUser,
  });
  ...
  if (ordersQuery.isError) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-brand-50 dark:bg-ink-900 p-4">
        <div className="w-full max-w-md rounded-lg bg-white p-8 text-center shadow-sm dark:bg-ink-800">
          <h2 className="mb-2 text-xl font-semibold text-ink-900 dark:text-white">Ошибка загрузки заказов</h2>
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        <div className="bg-white dark:bg-ink-800 rounded-lg shadow-sm p-6 mb-6">
          ...
          {ordersQuery.isLoading ? (
            <p className="text-ink-600 dark:text-ink-300">Загрузка заказов...</p>
          ) : (
                        ...
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            order.status === "DELIVERED"
                              ? "text-brand-700 dark:text-brand-300 bg-brand-100 dark:bg-ink-800"
